...
import org.newdawn.asteroids.gui.ClickableText;
...
public class MenuState implements GameState {
	...
	private ArrayList<ClickableText> textFields = new ArrayList<ClickableText>();
	...
	public void render(GameWindow window, int delta) {
		...
		for (int i=0;i<options.length;i++) {
			...
			if(textFields.size() != options.length){
				ClickableText text = new ClickableText(options[i], 270, 280+(i*40), 
						font.getCharHeight(), font.getCharWidth());
				textFields.add(text);
			}
	...
	public void update(GameWindow window, int delta) {
		...
		for(int i = 0; i < textFields.size(); i++){
			if(textFields.get(i).isInFocus(mouseX, mouseY))
				selected = i;
		}
