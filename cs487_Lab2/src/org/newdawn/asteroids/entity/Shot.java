...
public class Shot extends AbstractEntity implements Entity {
	...
	public Shot(Texture texture, float x, float y, float vx, float vy) {
		...
		particles = new ParticleGroup(100,200,1,1,0);
	}
	
	public Shot(Texture texture, float x, float y, float vx, float vy, float s) {
		positionX = x;
		positionY = y;
		velocityX = vx;
		velocityY = vy;
	...
	public void collide(EntityManager manager, Entity other) {
		...
		if (other instanceof Rock) {
			...
			if(!manager.isPowerUpActive("ARMOR-PEIRCING"))
				manager.removeEntity(this);
		}
	}
