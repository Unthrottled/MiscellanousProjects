...
public class Player extends AbstractEntity {
	...
	public Player(Texture texture, ObjModel model, Texture shotTexture) {
		...
		engine = new ParticleGroup(100,200,0.56f,0.2f,0);
	}
	
	public void setPowerUpTextures(Texture armor){
		armourPeircingRoundTexture = armor;
	}
	/**
	...
	public void update(EntityManager manager, int delta) {
		...
		if(shotTimeout == shotInterval && manager.isPowerUpActive("RAPID-FIRE"))
			shotTimeout = 50;
		shotTimeout -= delta;
		if (shotTimeout <= 0) {
		...
		/*if (shotTimeout <=0){
			if(Mouse.isButtonDown(Mouse.getEventButton())){
				//rotationZ = (float) Math.atan2(Mouse.getY() - positionY,
				//		Mouse.getX() - positionX);
		...
		if (Keyboard.isKeyDown(Keyboard.KEY_W) || Keyboard.isKeyDown(Keyboard.KEY_UP)) {
			...
			velocityX += (forwardX * delta) / 75.0f;
			velocityY += (forwardY * delta) / 75.0f;
		...
		/*if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
			float incrementX, incrementY, velX, velY;
			incrementX = Math.abs((forwardX * delta) / 75.0f);
			incrementY = Math.abs((forwardY * delta) / 75.0f);
			velX = Math.abs(velocityX);
			velY = Math.abs(velocityY);
			System.out.println(velX + " " + velY);
			if(velX > 0  &&  velX > velY){
				if(Math.abs(incrementX) > Math.abs(velocityX))
					velocityX = 0;
				else{
					if(velocityX < 0)
			...
			else if(velY > 0 && velY > velX){
				if(Math.abs(incrementY) > Math.abs(velocityY))
					velocityY = 0;
				else{
					if(velocityY < 0)
		...
		engine.update(delta);
		if(kineticBarrier != null)
			kineticBarrier.setPosition(positionX, positionY, rotationZ);
	}
	...
	private void fire(EntityManager manager) {
		...
		Shot shot;
		if(manager.isPowerUpActive("BIG-SHOT"))
			shot = new Shot(shotTexture, 
							 getX() + forwardX, 
							 getY() + forwardY, 
							 forwardX * 30, 
							 forwardY * 30, 3.5f);
		else if(manager.isPowerUpActive("ARMOR-PEIRCING") && 
				armourPeircingRoundTexture != null)
			shot = new Shot(armourPeircingRoundTexture, 
					 getX() + forwardX, 
					 getY() + forwardY, 
					 forwardX * 30, 
					 forwardY * 30);
	...
	public void setKineticBarrier(OverShield barrier){
		kineticBarrier = barrier;
	}
	...
	public void collide(EntityManager manager, Entity other) {
		...
		if (other instanceof Rock ) {			
			...
			if(!manager.isPowerUpActive("INVINCIBLITY")){
			...
			if(!((PowerUp) other).isCollected())
				((PowerUp) other).collected(manager, this);
		}
		/*
